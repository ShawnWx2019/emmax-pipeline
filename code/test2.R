...
server <- function(input,output,session) {
  ...
  output$down_tbl_00 = downloadHandler(
    ...
    content = function(file) {
      writexl::write_xlsx(x = ht_obj$phenotable2,path = filename)
    }
  )
  ...
  output$down_tbl_01 = downloadHandler(
    ...
    content = function(file) {
      writexl::write_xlsx(x = DA_obj$hap_res$DA_result,path = filename)
    }
  )
  ...
  output$down_tbl_02 = downloadHandler(
    ...
    content = function(file) {
      writexl::write_xlsx(x = DA_obj$hap_res$tbl,path = filename)
    }
  )
  ...
  output$down_tbl_03 = downloadHandler(
    ...
    content = function(file) {
      writexl::write_xlsx(x = snp_obj$snps_tbl$geno_tbl2,path = filename)
    }
  )
